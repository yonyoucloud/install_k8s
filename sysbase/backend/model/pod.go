...
func (p Pod) InitTable() {
	// 设置表信息
	migrator := db.Set("gorm:table_options", "ENGINE=InnoDB CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Pods表'").Migrator()

	// 判断表是否存在
	if migrator.HasTable(&Pod{}) {
		// 存在就自动适配表，也就说原先没字段的就增加字段
		migrator.AutoMigrate(&Pod{})
	} else {
		// 不存在就创建新表
		migrator.CreateTable(&Pod{})
	}
}
...
func (p Pod) List() ([]Pod, error) {
	var ps []Pod
	result := db.Order("id desc").Find(&ps)
...
func (p Pod) Get() (Pod, error) {
	var pData Pod
	err := db.Where(&p).Find(&pData).Error
	return pData, err
}
